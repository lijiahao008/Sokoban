...
function detect_key(key){
	...
	if (!startedListen && key_code==32) {
		...
		audioStatus.innerHTML = "<i class='fa fa-microphone'></i>  Listening..."
		let sr = startListen();
		startedListen = true;
	}
}
...
function detect_key_up(event) {
	if (event.keyCode == 32) {
		...
		audioStatus.innerHTML = "<i class='fa fa-microphone'></i> Press and Hold space to speak"
		sr.stop();
		startedListen = false;
	}
}
...
function nextLevel() {
	document.getElementById("next-level").blur();
